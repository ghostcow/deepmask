...
local lDataSet = torch.class('DataSet')
...
function lDataSet:append(...)
   ...
   for k,file in pairs(files) do
	 ...
	 print(pathToPpm)
	  ...
	  if (input:size(1) == channels) then -- 3D tensor
	   imageHeight = input:size(2)
	   imageWidth = input:size(3)
	  else -- 2D tensor height x width
	   imageHeight = input:size(1)
	   imageWidth = input:size(2)
	 end
         table.remove(rawOutput,1) --remove file ID

         -- put input in 3D tensor
         input:resize(channels, imageHeight, imageWidth)
