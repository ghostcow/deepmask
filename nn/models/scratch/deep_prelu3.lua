...
function createModel()
    ...
    model:add(nn.PReLU())
    model:add(cudnn.SpatialConvolution(64, 48, 1, 1, 1, 1)) -- 11
    model:add(nn.PReLU())
    ...
    -- Conv41 & Conv42
    model:add(cudnn.SpatialConvolution(48, 64, 3, 3, 1, 1, 1, 1)) -- 13
    model:add(nn.PReLU())
    model:add(cudnn.SpatialConvolution(64, 80, 3, 3, 1, 1, 1, 1))
    model:add(nn.PReLU())
    model:add(cudnn.SpatialConvolution(80, 96, 3, 3, 1, 1, 1, 1))
    model:add(nn.PReLU())
    model:add(cudnn.SpatialConvolution(96, 80, 1, 1, 1, 1)) -- 11
    model:add(nn.PReLU())
    ...
    -- Fc6
    model:add(nn.Reshape(128,true))
    model:add(nn.Linear(128, #dataset.classes))
